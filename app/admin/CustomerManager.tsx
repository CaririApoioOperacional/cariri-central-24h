...
export default function CustomerManager(){
 ...
 return <div className="admin-grid">
  ...
  <section className="wide"><h3>Base cadastrada</h3><div className="list">{rows.map(c=><article key={c.id}><b>{c.name}</b><small>{c.sites.length} local(is) • {c.contacts.length} WhatsApp(s)</small>{c.sites.map(x=><small key={x.id}>• {x.name}{x.address?" — "+x.address:""}{x.latitude!=null&&x.longitude!=null?" • GPS configurado":""}</small>)}{c.contacts.map(x=><small key={x.id}>• WhatsApp {x.phone}{x.name?" — "+x.name:""}</small>)}</article>)}</div></section>
 </div>
}
