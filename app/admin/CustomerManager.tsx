...
export default function CustomerManager(){
 ...
 const [customerId,setCustomerId]=useState(""),[siteName,setSiteName]=useState(""),[address,setAddress]=useState("");
 ...
 async function addSite(e:FormEvent){e.preventDefault();if(await send("/api/sites",{customerId,name:siteName,address})){setSiteName("");setAddress("")}}
 ...
 const sites=rows.find(x=>x.id===contactCustomer)?.sites||[];
 return <div className="admin-grid">
  ...
  <section className="wide"><h3>Base cadastrada</h3><div className="list">{rows.map(c=><article key={c.id}><b>{c.name}</b><small>{c.sites.length} local(is) • {c.contacts.length} WhatsApp(s)</small>{c.sites.map(x=><small key={x.id}>• {x.name}{x.address?" — "+x.address:""}</small>)}{c.contacts.map(x=><small key={x.id}>• WhatsApp {x.phone}{x.name?" — "+x.name:""}</small>)}</article>)}</div></section>
 </div>
}
